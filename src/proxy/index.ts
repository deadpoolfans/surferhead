import http, { ServerOptions } from "node:http"; // Fully compatible with Bun
import https from "node:https"; // Fully compatible with Bun
import dns from "node:dns"; // Fully compatible with Bun

// Missing `net.{get|set}DefaultAutoSelectFamily`, `net.SocketAddress` and `net.BlockList`.
// Should be compatible since we don't use them though.
import net from "node:net";

import type { ExternalProxySettingsRaw } from "../typings/session";
...
export default class Proxy extends Router {
    ...
    constructor (options: { staticContentCaching?: Record<string, unknown> }) {
      super(options);
    ...
    _getOptions (ssl?: Record<string, unknown>): ServerOptions {
      const options = (ssl ?? {}) as ServerOptions;

      options.maxHeaderSize = Proxy.MAX_REQUEST_HEADER_SIZE;
      return options;
    }
    ...
    async _onServiceMessage (req: http.IncomingMessage, res: http.ServerResponse, serverInfo: ServerInfo): Promise<void> {
      ...
      const msg     = toServiceMessage(body);
    ...
    async _onServiceWebSocketMessage (ws, data, serverInfo): Promise<void> {
      const msg     = toServiceMessage(data);
      const session = msg && this.openSessions.get(msg.sessionId);
    ...
    start (options: ProxyOptions) {
      ...
      const opts         = this._getOptions(ssl);
